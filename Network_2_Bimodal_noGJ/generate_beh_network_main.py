...
def create_GoC_network( duration=1000, dt=0.025, seed=100, runid=0, mf=0.2, pf=0.2, mf2=0.2, pf2=0.2, hom=True, run=False):
	...
	simid = 'biNet_noGJ_'+format(runid,'05d')+'_mf_'+format(int(mf*100),'03d')+'_pf_'+format(int(pf*100),'03d')+'_mf2_'+format(int(mf2*100),'03d')+'_pf2_'+format(int(pf2*100),'03d')+'_'+hom_str

	net = nml.Network( id=simid, type="networkWithTemperature" , temperature="23 degC" )
	net_doc = nml.NeuroMLDocument( id=net.id )
	net_doc.networks.append( net )

	arr_flnm = ['../Mechanisms/SpikeArray_MFON_dur_060_080.nml', '../Mechanisms/SpikeArray_MFOFF_dur_060_080.nml']
...
def get_params( runid=0, mf=0.2, pf=0.2, mf2=0.1, pf2=0.1, hom=False ):
	...
	nInputs_max = { "MF_bg": nMF, "PF_bg" : nPF, "MFON" : nMF, "PFON": nPF , "MFOFF" : nMF, "PFOFF": nPF }
	nInputs_frac = { "MF_bg": 1.0, "PF_bg" : 1.0, 'MFON': mf, 'PFON' : pf, 'MFOFF': mf2, 'PFOFF' : pf2}

	params = inp.get_simulation_params( runid, nInputs_max=nInputs_max, nInputs_frac=nInputs_frac, nGoC_pop = nPop, Input_prob=Input_prob, stepInputs=[] )
	return params
